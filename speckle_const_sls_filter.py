...
package. This one applies a constant value for the Brownian SLS
filter. THIS IS MORE DANGEROUS! Make sure you choose the value
...
def main(args: List[str]) -> int:
    ...
    print('This program will recursively walk through a given',
          'folder and apply a constant SLS threshold to every',
    ...
    if len(args) != 3:
        print('Please provide 2 command-line arguments:',
              'The root folder, and the SLS threshold.')
        return 1

    root: str = args[1]
    threshold: float = float(args[2])
    ...
    def filter_folder(dir_path: str) -> None:
        ...
        def filter_single_file(file_path: str) -> None:
            ...
            nonlocal total_dropped, total_remaining
            ...
            contents: s.FreqFile = \
                s.load_frequency_file(file_path)

            # Apply Brownian filter
            dropped, remaining = contents.filter(
                f.sls_threshold_filter, sls_threshold=threshold)
        ...
        s.for_each_file(filter_single_file, dir_path,
                        r'.*track.*\.csv')
    ...
    try:
        ...
        if files_skipped:
            print('Overfiltering caused the skippage of',
                  f'{len(files_skipped)} files out of',
                  f'{len(files_done)}')
            print('Skipped files:')
